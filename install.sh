...
bitcoinnew Masternode installer arguments:
...
# Set these to change the version of bitcoinnew to install
TARBALLURL="https://github.com/btcncoin/BitcoinNew/releases/download/1.0.0/bitcoinnew-linux.tar.gz"
TARBALLNAME="bitcoinnew-linux.tar.gz"
BOOTSTRAPURL=""
BOOTSTRAPARCHIVE=""
BWKVERSION="1.0.1"
...
echo "
 ...
 | the BTCN Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "bitcoinnew"' && echo ""
sleep 1
...
# Install bitcoinnew daemon
wget $TARBALLURL
tar -xzvf $TARBALLNAME 
rm $TARBALLNAME
mv ./bitcoinnewd /usr/local/bin
mv ./bitcoinnew-cli /usr/local/bin
mv ./bitcoinnew-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .bitcoinnew directory
mkdir $USERHOME/.bitcoinnew
...
touch $USERHOME/.bitcoinnew/bitcoinnew.conf
cat > $USERHOME/.bitcoinnew/bitcoinnew.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
addnode=139.180.217.0
addnode=139.180.133.164
addnode=202.182.102.177
addnode=66.42.48.83

EOL
chmod 0600 $USERHOME/.bitcoinnew/bitcoinnew.conf
chown -R $USER:$USER $USERHOME/.bitcoinnew

sleep 1

cat > /etc/systemd/system/bitcoinnew.service << EOL
[Unit]
...
ExecStart=/usr/local/bin/bitcoinnewd -conf=${USERHOME}/.bitcoinnew/bitcoinnew.conf -datadir=${USERHOME}/.bitcoinnew
ExecStop=/usr/local/bin/bitcoinnew-cli -conf=${USERHOME}/.bitcoinnew/bitcoinnew.conf -datadir=${USERHOME}/.bitcoinnew stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable bitcoinnew.service
sudo systemctl start bitcoinnew.service
